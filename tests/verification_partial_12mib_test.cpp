...
int main()
{
    ...
    assert(verification.verifyPartial12MiB(
        image,
    ...
    assert(verified_offsets.size() == 192);
    assert(verified_lengths.size() == 192);
    ...
    assert(verified_offsets.back() == 0x00BF0000);
    ...
    try {
        ...
        (void)invalid_session.verifyPartial12MiB(
