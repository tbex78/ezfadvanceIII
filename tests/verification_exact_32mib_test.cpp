...
int main()
{
    constexpr std::size_t image_size =
        ezfadvance::VerificationSession::four_window_size;
    ...
    for (std::size_t i = 0; i < image.size(); ++i) {
        ...
        image[i] = static_cast<std::uint8_t>(
            (within_block * 47u + block * 79u) & 0xFFu);
    }
    ...
    assert(verification.verifyExact32MiB(
        image,
        [&](std::size_t offset, std::size_t length) {
            verified_offsets.push_back(offset);
            verified_lengths.push_back(length);
        }));
    assert(verified_offsets.size() == 512);
    assert(verified_lengths.size() == 512);
    ...
    assert(verified_offsets.back() == 0x01FF0000);
    ...
    try {
        ...
        (void)invalid_session.verifyExact32MiB(
