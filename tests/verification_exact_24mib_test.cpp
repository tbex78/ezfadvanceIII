...
int main()
{
    constexpr std::size_t image_size =
        ezfadvance::VerificationSession::three_window_size;
    ...
    for (std::size_t i = 0; i < image.size(); ++i) {
        ...
        image[i] = static_cast<std::uint8_t>(
            (within_block * 31u + block * 73u) & 0xFFu);
    }
    ...
    assert(verification.verifyExact24MiB(
        image,
        [&](std::size_t offset, std::size_t length) {
            verified_offsets.push_back(offset);
            verified_lengths.push_back(length);
        }));
    assert(verified_offsets.size() == 384);
    assert(verified_lengths.size() == 384);
    ...
    assert(verified_offsets.back() == 0x017F0000);
    assert(transcript.complete());

    assert(delays == std::vector<std::chrono::microseconds>{
                         std::chrono::microseconds(125000)});
    ...
    try {
        ...
        (void)invalid_session.verifyExact24MiB(
