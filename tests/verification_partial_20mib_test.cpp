...
int main()
{
    ...
    assert(verification.verifyPartial20MiB(
        image,
    ...
    assert(verified_offsets.size() == 320);
    assert(verified_lengths.size() == 320);
    ...
    assert(verified_offsets.back() == 0x013F0000);
    ...
    try {
        ...
        (void)invalid_session.verifyPartial20MiB(
